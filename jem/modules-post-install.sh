...
[ $? -ne 0 ] && exit 1
make -j8 -C $COMMON_FOLDER/pvr-source/eurasiacon/build/linux2/omap4430_android ARCH=arm KERNEL_CROSS_COMPILE="$CCACHE $ARM_EABI_TOOLCHAIN/arm-eabi-" CROSS_COMPILE="$CCACHE $ARM_EABI_TOOLCHAIN/arm-eabi-" KERNELDIR=$KERNEL_OUT TARGET_PRODUCT="blaze_tablet" BUILD=release TARGET_SGX=544sc PLATFORM_VERSION=4.0
[ $? -ne 0 ] && exit 1
