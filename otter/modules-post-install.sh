...
[ $? -ne 0 ] && exit 1
make -j8 -C $CM_ROOT/hardware/ti/wlan/mac80211/compat_wl12xx KERNEL_DIR=$KERNEL_OUT KLIB=$KERNEL_OUT KLIB_BUILD=$KERNEL_OUT ARCH=arm CROSS_COMPILE="$CCACHE $ARM_EABI_TOOLCHAIN/arm-eabi-"
[ $? -ne 0 ] && exit 1
...
[ $? -ne 0 ] && exit 1
make -j8 -C $COMMON_FOLDER/pvr-source/eurasiacon/build/linux2/omap4430_android ARCH=arm KERNEL_CROSS_COMPILE="$CCACHE $ARM_EABI_TOOLCHAIN/arm-eabi-" CROSS_COMPILE="$CCACHE $ARM_EABI_TOOLCHAIN/arm-eabi-" KERNELDIR=$KERNEL_OUT TARGET_PRODUCT="blaze_tablet" BUILD=release TARGET_SGX=540 PLATFORM_VERSION=4.0
[ $? -ne 0 ] && exit 1
